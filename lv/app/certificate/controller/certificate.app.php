	...
	private function apply()
	{
		...
		if($this->ev->get('apply'))
		{
			...
			foreach($ce['cebasic'] as $basicid)
			{
				if($basicid)
				{
					$eh = $this->G->make('favor','exam')->getExamHistoryByArgs(array(array("AND","ehuserid = :ehuserid","ehuserid",$this->_user['sessionuserid']),array("AND","ehispass = 1"),array("AND","ehtype = 2"),array("AND","ehbasicid = :ehbasicid","ehbasicid",$basicid)));
					if(!$eh['ehid'])
					{
						$message = array(
							'statusCode' => 300,
