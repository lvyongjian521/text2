...
elseif($act == 'goodslist')
{
	...
	while($res = $DB->fetch($rs)){
		$data[] = array('tid' => $res['tid'] , 'name' => $res['name'] , 'shopimg' => $res['shopimg'] , 'close' => $res['close']);
	}
	exit(json_encode($data));
}
elseif($act == 'pay')
{
	...
	if ($tool && $tool['active'] == 1) {
		...
		if ($userrow && $userrow['user'] == $user && $userrow['pwd'] == $pass) {
			$result['code'] = 0;
			...
			$price_obj = new Price($userrow['zid'],$siterow);
		...
		} elseif ($userrow && $userrow['active'] == 0) {
