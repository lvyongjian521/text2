...
foreach($msgrow as $row){
	...
	/*echo '
				<div class="panel panel-default">
					<div class="panel-heading"><h3 class="panel-title">'.($row['read']?'<span class="label label-success">已读</span>':'<span class="label label-warning">未读</span>').'&nbsp;<a data-toggle="collapse" href="#collapse'.$row['id'].'"><b>'.$row['title'].'</b></a></h3></div>
					<div id="collapse'.$row['id'].'" class="panel-collapse collapse in">
					<div class="panel-body">
					<small><font color="grey">管理员  发送于'.$row['addtime'].'</font></small><br><br>
					'.$row['content'].'
					</div>
					</div>
				</div>';*/
